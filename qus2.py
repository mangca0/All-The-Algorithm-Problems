...
# 添加函数：计算进入CJ1时刻和离开CJ4时刻
def calculate_schedule_times(sequence, processing_times):
    """
    计算每个疫苗的进入CJ1时刻和离开CJ4时刻
    :param sequence: 加工顺序（1-10的序列）
    :param processing_times: 工件加工时间矩阵
    :return: 每个疫苗的进入CJ1时刻和离开CJ4时刻
    """
    n = len(sequence)
    sequence_idx = [x - 1 for x in sequence]  # 转换为0-9的索引

    # 初始化时间数组
    start_times = [0] * n  # 进入CJ1时刻
    end_times = [0] * n  # 离开CJ4时刻

    # 工位完成时间
    station_times = [0] * 4  # CJ1到CJ4的完成时间
    ...
    for i in range(n):
        ...
        if i == 0:
            ...
            start_times[i] = station_times[0]  # 前一个疫苗完成CJ1的时间

        # 更新每个工位的完成时间
        station_times[0] = start_times[i] + processing_times[idx][0]
        for j in range(1, 4):  # 工位 CJ2 到 CJ4
            station_times[j] = max(station_times[j], station_times[j - 1]) + processing_times[idx][j]

        # 记录离开CJ4时刻
        end_times[i] = station_times[3]

    return start_times, end_times

def hours_to_time_format(hours):
    """
    将小时数转换为"时:分"格式
    例如：3.5小时 -> "03:30"
    """
    total_minutes = int(hours * 60)  # 将小时转换为分钟
...
if __name__ == "__main__":
    ...
    # 将numpy数组转换为普通列表
    best_schedule = [int(x) for x in best_schedule]
    ...
    # 计算进入CJ1时刻和离开CJ4时刻
    start_times, end_times = calculate_schedule_times(best_schedule, processing_times)

    # 输出表格
    print("\n疫苗编号\t进入CJ1时刻\t离开CJ4时刻")
    for i in range(len(best_schedule)):
        start_time = hours_to_time_format(start_times[i])
        end_time = hours_to_time_format(end_times[i])
        print(f"{best_schedule[i]}\t\t{start_time}\t\t{end_time}")